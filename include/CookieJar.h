...
namespace CppWebSpider{
	...
	*/
	class CookieJar{
	public:
		CookieJar():cookie_map(std::unordered_map<std::string,std::vector<cookie_struct> >()){}
		...
		*/
		void add(std::string host,std::vector<cookie_struct> cookie_list);
		...
		/*
			...
			uri		要爬取的资源
		*/
		std::string use(std::string host,std::string uri);
	...
	private:
		...
		*/
		void add_helper(std::string host,cookie_struct cs);
		/*
			...
			uri		当前的uri
		*/
		std::string use_helper(std::string host,std::string uri);
		//所有使用了这个CookieJar的cookie的保存容器
		std::unordered_map<std::string,std::vector<cookie_struct>> cookie_map;
	};
}
#endif